...
```{r}
sales_sdf %>%
  withColumn("dt", unix_timestamp(.$date, 'dd.MM.yyyy')) -> new_sdf
...
new_sdf %>%
  ...
  geom_line(colour="darkorange") +
  theme_minimal()
# the first argument of ggplot is taken as all the data being piped with %>% to it.
```

# or in a simplified way, w/o resorting to creating a new data frame...
# although in Spark a new data frame only references the data, it does not copy it.
...
The data consists, partitioned by date and shop_id is:
    shop_id | date | item_id | item_cnt_day | total_items
       0       0       0          1              3     (1st window)
       0       0       1          2              3     (1st window)
       0       1       0          2              2     (2nd window)
       1       0       0          3              7     (3rd window)
       1       0       1          4              7     (3rd window)
...
" %>%
  sql %>%
  head
# date_block_num is a number mapped to a date.
```
...
" %>%
  sql %>%
  head
# explode permits exploding records that contain certain atributes along with an array,
# transforming each array entry with other constant atributes in a new records 
